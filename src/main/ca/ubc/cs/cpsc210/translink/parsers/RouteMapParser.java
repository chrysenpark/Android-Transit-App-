...
public class RouteMapParser {
    ...
    private void parseOnePattern(String str) {
        ...
        String routeNumber = str.split(";")[0].substring(1, str.indexOf("-"));
        String patternNumber = str.split(";")[0].substring(str.indexOf(("-")));
        String[] numbers = str.split(";");
        List<LatLon> latLonList = new ArrayList<>();

        for (int i = 1; i < (numbers.length) ; i += 2) {
            double lat = Double.parseDouble(numbers[i]);
            double lon = Double.parseDouble(numbers[i+1]);
            LatLon tuple = new LatLon(lat,lon);
            latLonList.add(tuple);
        }

        storeRouteMap(routeNumber,patternNumber,latLonList);
