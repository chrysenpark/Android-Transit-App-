...
public class ArrivalsParser {
    ...
    public static void parseArrivals(Stop stop, String jsonResponse)
        ...
        // Route no
        // Expected Countdown, Destination, ScheduleStatus
        ...
        JSONArray jsonArray = new JSONArray(jsonResponse);
            ...
            for (int i = 0;i < jsonArray.length(); i++) {
                ...
                parseSchedule(stop, route, obj.getJSONArray("Schedules"));
        }

    }

    private static void parseSchedule(Stop stop, Route route, JSONArray ScheduleObj) throws JSONException {

        for (int i = 0; i < ScheduleObj.length(); i++) {
            JSONObject obj = ScheduleObj.getJSONObject(i);
            int ExpectedCountdown = obj.getInt("ExpectedCountdown");
            String Destination = obj.getString("Destination");
            String ScheduleStatus = obj.getString("ScheduleStatus");
